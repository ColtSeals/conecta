        ...
        <div class="modal-body">
            <div class="row gy-4">
                ...
                <div class="form-check custom-option custom-option-basic d-flex justify-content-between mt-3">
                    ...
                    @foreach($rolesCollection->sortBy(function($role, $roleId) {
                        return \App\Models\Role::find($roleId)->order;
                    }) as $roleId => $checked)
                        @php
                            $role = \App\Models\Role::find($roleId);
                        @endphp
                        <label class="form-check-label custom-option-content" for="customCheck{{ $roleId }}">
                            <input class="form-check-input" type="checkbox" wire:model="roles.{{ $roleId }}" id="customCheck{{ $roleId }}" {{ $checked ? 'checked' : '' }}>
                            <span class="custom-option-body">
                                <small class="option-text">{{ $rolesTranslations[$role->name] }}</small>
                            </span>
                        </label>
                    @endforeach
